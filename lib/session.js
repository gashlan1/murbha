...
/** Extract userId from req cookie, or null. Honors per-user session revocation. */
const userIdFromReq = (req) => {
  const cookies = parseCookies(req.headers.cookie);
  const payload = verify(cookies[COOKIE_NAME]);
  if (!payload?.uid) return null;
  // If the user has revoked all sessions after some moment, reject tokens
  // issued before then. Tokens without iat (older than this change) are
  // always honored unless an explicit revocation is set after their exp.
  try {
    const u = db.find('users', x => x.id === payload.uid);
    if (u?.sessionsInvalidAfter && payload.iat &&
        payload.iat < new Date(u.sessionsInvalidAfter).getTime()) return null;
  } catch (e) {}
  return payload.uid;
};
...
const publicUser = (u) => {
  if (!u) return null;
  const { passwordHash, salt, emailOtp, mobileOtp, passwordReset, totpSecret, ...safe } = u;
  return safe;
};
...
const setSessionCookie = (res, userId) => {
  ...
  const token = sign({ uid: userId, iat, exp });
...
const clearSessionCookie = (res) => {
  ...
  res.setHeader('Set-Cookie', [
    ...
    `${CSRF_COOKIE}=; Path=/; SameSite=Lax; Max-Age=0${isProd ? '; Secure' : ''}`,
  ]);
};

// ─── CSRF (double-submit cookie) ──────────────────────────────────
// On any GET, ensure a non-HttpOnly CSRF cookie exists. The frontend
// reads it via JS and sends back as X-CSRF-Token on state-changing
// requests. Server compares header to cookie with timing-safe equality.
// Combined with SameSite=Lax on the session cookie, this defeats CSRF.
const setCsrfCookieIfMissing = (req, res) => {
  const existing = parseCookies(req.headers.cookie)[CSRF_COOKIE];
  if (existing) return existing;
  const token = crypto.randomBytes(24).toString('hex');
  const isProd = process.env.NODE_ENV === 'production';
  res.setHeader('Set-Cookie',
    `${CSRF_COOKIE}=${token}; Path=/; SameSite=Lax; Max-Age=${COOKIE_MAX_AGE}${isProd ? '; Secure' : ''}`
  );
  return token;
};
