...
R('PATCH', /^\/app\/admin\/projects\/([^/]+)$/, async (req, res, m) => {
  ...
  if (!p) return err(res, 404, 'not_found');
  const body = await readJson(req);
  const allowed = ['name','category','city','summary','description','image','status','goal','minAmount','termMonths','profitRate','closesAt','guarantee','shariaApproval','fatwaId','inviteOnly','paused','sponsorVerified'];
  const changes = {};
  for (const k of allowed) if (k in (body || {})) changes[k] = body[k];
