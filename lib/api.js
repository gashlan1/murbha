...
R('POST', '/app/auth/login', async (req, res) => {
  ...
  if (!user) return err(res, 401, 'bad_credentials');
  if (!S.verifyPassword(password, user.salt, user.passwordHash)) return err(res, 401, 'bad_credentials');
  if (user.rejected) return err(res, 403, 'account_rejected');
...
R('GET', /^\/app\/projects\/([^/]+)$/, async (req, res, m) => {
  ...
  return send(res, 200, { project: { ...p, fundedPct } });
});
...
R('POST', '/app/auth/verify-email', async (req, res) => {
  ...
  await db.patch('users', u => u.id === user.id, { kycStatus: 'email_verified' });
...
R('POST', '/app/auth/verify-mobile', async (req, res) => {
  ...
  await db.patch('users', u => u.id === user.id, { kycStatus: 'mobile_verified' });
  return send(res, 200, { success: true });
});
