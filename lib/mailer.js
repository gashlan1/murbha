...
const passwordResetTemplate = (name, resetUrl) => {
  const innerHtml = `
    <p style="margin:0 0 12px;font-size:15px;line-height:1.7;">مرحباً ${name || ''}،</p>
    <p style="margin:0 0 20px;font-size:15px;line-height:1.7;">تلقّينا طلباً لإعادة تعيين كلمة المرور لحسابك على منصة مُرابحة. اضغط على الزر أدناه لإنشاء كلمة مرور جديدة:</p>
    <div style="text-align:center;margin:0 0 24px;">
      <a href="${resetUrl}" style="background:#062b1e;color:#fbf6ea;padding:14px 28px;border-radius:12px;text-decoration:none;font:800 14px 'Cairo',sans-serif;display:inline-block;">إعادة تعيين كلمة المرور</a>
    </div>
    <p style="margin:0 0 8px;font-size:13px;color:#6b7268;line-height:1.7;">هذا الرابط صالح لمدة ساعة واحدة فقط ولا يمكن استخدامه إلا مرة واحدة.</p>
    <p style="margin:16px 0 0;font-size:12px;color:#6b7268;line-height:1.6;">إذا لم تطلب إعادة تعيين كلمة المرور، يمكنك تجاهل هذه الرسالة. كلمة مرورك ستبقى دون تغيير.</p>
    <p style="margin:14px 0 0;font-size:11px;color:#6b7268;line-height:1.5;word-break:break-all;">إذا لم يعمل الزر، انسخ هذا الرابط في المتصفح:<br/>${resetUrl}</p>
  `;
  const text = `مرحباً ${name || ''}،\n\nتلقّينا طلباً لإعادة تعيين كلمة المرور.\nافتح هذا الرابط لاختيار كلمة مرور جديدة:\n${resetUrl}\n\nصالح لمدة ساعة واحدة، لمرة واحدة فقط.\nإذا لم تطلب ذلك، يمكنك تجاهل الرسالة.\n\n— منصة مُرابحة`;
  return { html: wrapper(innerHtml), text };
};
